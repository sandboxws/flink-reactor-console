 ...
 *
 * `generateDml` is the public entry; the internal helpers handle:
 *   - direct sinks (collectSinkDml — both reverse-nesting and forward-
 *     reading JSX patterns)
 *   - StatementSet (transparent passthrough)
 *   - Route + Route.Branch / Route.Default (collectRouteDml; one INSERT
 *     per branch, with optional condition merged into the lead Filter)
 *   - SideOutput / Validate (delegated to sql-query-side-paths.ts)
 *   - rowtime-column demotion (wrapSinkQueryForMultiRowtime — BUG-029)
 *   - FileSystemSink partition-column projection (uses
 *     `resolvePartitionExpression` from sql-ddl-sink.ts)
 */
...
function buildSiblingChainQuery(
  ...
  for (let i = 1; i < chain.length; i++) {
    ...
    if (transform.kind === "Window" && transform.children.length > 0) {
      ...
      currentUpstream = {
        sql: ctx.buildQuery(ctx, transform),
    ...
    const sql = ctx.buildQuery(ctx, transform)
...
function collectSinkDml(
  ...
  if (node.kind === "Sink") {
    ...
    if (node.children.length > 0) {
      ...
      upstream = ctx.buildQuery(ctx, node.children[0])
    } else if (parent) {
