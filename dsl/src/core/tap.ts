...
export function resolveObservationStrategy(
...
function resolveConnectorContext(node: ConstructNode): {
  ...
  // Sources have their own connector properties and schema
  if (node.kind === "Source") {
    return {
  ...
  // Sinks have their own connector properties but no schema prop —
  // walk upstream to find the schema from the feeding source/transform.
  if (node.kind === "Sink") {
    const connectorProps = extractConnectorProperties(node)
    const upstreamSource = findUpstreamSourceNode(node)
    const schema = upstreamSource ? extractNodeSchema(upstreamSource) : {}
...
/**
 * Walk upstream through children to find the nearest Source node with a schema.
 * Used by sinks which need their upstream source's schema for tap observation.
 */
function findUpstreamSourceNode(node: ConstructNode): ConstructNode | null {
  for (const child of node.children) {
    if (child.kind === "Source") return child
    const found = findUpstreamSourceNode(child)
    if (found) return found
  }
  return null
}
