...
export const PROP_METADATA: Record<string, readonly PropMeta[]> = {
  ...
  "JdbcSink": [
    ...
    {"name":"onConflict","required":false,"type":"\"DO NOTHING\" | \"DO ERROR\" | \"DO DEDUPLICATE\"","enumValues":["DO NOTHING","DO ERROR","DO DEDUPLICATE"],"doc":"Upsert conflict strategy → trailing `ON CONFLICT DO …` clause on the\ngenerated INSERT (Flink 2.3+, FLIP-558). Only meaningful for upsert sinks."},
    {"name":"parallelism","required":false,"type":"number"},
    {"name":"tap","required":false,"type":"boolean | TapConfig","doc":"Enable operator tailing for this sink"},
  ...
  "LookupJoin": [
    ...
    {"name":"on","required":true,"type":"string","doc":"Join key. A bare column name (`\"customer_id\"`) is treated as an\nequi-join on that column present on both sides; a full condition\n(`\"o.customer_id = c.customer_id\"`) is emitted verbatim."},
    {"name":"async","required":false,"type":"LookupAsyncConfig","doc":"Async lookup configuration"},
    {"name":"cache","required":false,"type":"LookupCacheConfig$1","doc":"Lookup cache configuration (inline form)"},
    {"name":"dimension","required":false,"type":"ConstructNode","doc":"Dimension table as a source node (e.g. a `JdbcSource`). Provide this\nOR the inline `table` + `url` + `schema` props — the inline form is\nsugar that builds an equivalent `JdbcSource` for you."},
    {"name":"parallelism","required":false,"type":"number"},
    {"name":"schema","required":false,"type":"SchemaDefinition<Record<string, FlinkType>>","doc":"Dimension table schema — columns + primary key (inline form)"},
    {"name":"select","required":false,"type":"Record","doc":"Output field mapping"},
    {"name":"table","required":false,"type":"string","doc":"Dimension table name (inline form; ignored when `dimension` is set)"},
    {"name":"tap","required":false,"type":"boolean | TapConfig","doc":"Enable operator tailing for this join"},
    {"name":"url","required":false,"type":"string","doc":"JDBC connection URL for the dimension table (inline form)"},
  ],
  "Map": [
  ...
  "MaterializedTable": [
    ...
    {"name":"primaryKey","required":false,"type":"readonly string[]","doc":"Primary key column(s) → `PRIMARY KEY (...) NOT ENFORCED`. Flink 2.3+ (FLIP-550)."},
    ...
    {"name":"startMode","required":false,"type":"\"FROM_BEGINNING\" | \"FROM_NOW\" | \"RESUME_OR_FROM_BEGINNING\" | \"RESUME_OR_FROM_NOW\" | { readonly mode: \"FROM_TIMESTAMP\" | \"RESUME_OR_FROM_TIMESTAMP\"; readonly timestamp: string; }","doc":"Data-reprocessing start mode (Flink 2.3+, FLIP-557). Controls how the\nrefresh job seeds historical data: replay from the beginning, start from\nnow, or start from an explicit timestamp."},
    {"name":"watermark","required":false,"type":"{ readonly column: string; readonly expression: string; }","doc":"Watermark spec → `WATERMARK FOR <column> AS <expression>`. Flink 2.3+ (FLIP-550)."},
  ...
  "Pipeline": [
    ...
    {"name":"telemetry","required":false,"type":"TelemetryConfig"},
  ...
  "YugabyteCdcSource": [
    {"name":"database","required":true,"type":"string","doc":"Postgres database → `'database-name'`."},
    {"name":"hostname","required":true,"type":"string"},
    {"name":"password","required":true,"type":"SecretRef","doc":"Secret reference for the password. A plain string is rejected at compile\ntime — use `secretRef(...)`. Rendered as a `${env:<envName>}` placeholder\nin the WITH clause; the runtime adapters substitute it before submission\n(the docker adapter inlines from the host env, the kubernetes adapter binds\na `secretKeyRef` env in the FlinkDeployment podTemplate)."},
    {"name":"schema","required":true,"type":"SchemaDefinition<T>","doc":"The Flink table column schema (+ optional PK / watermark / metadata columns)."},
    {"name":"table","required":true,"type":"string","doc":"Postgres table → `'table-name'`."},
    {"name":"username","required":true,"type":"string"},
    {"name":"decodingPluginName","required":false,"type":"string","doc":"Logical-decoding plugin → `'decoding.plugin.name'`. Defaults to `'pgoutput'`,\nwhich YugabyteDB requires — its CDC fork does not support the upstream\ndefault (`decoderbufs`)."},
    {"name":"name","required":false,"type":"string","doc":"Optional SQL table name. Defaults to the Yugabyte table name."},
    {"name":"parallelism","required":false,"type":"number"},
    {"name":"port","required":false,"type":"number","doc":"Defaults to `5433` — the YugabyteDB YSQL tserver port (NOT Postgres 5432)."},
    {"name":"primaryKey","required":false,"type":"readonly string[]","doc":"Explicit PK hint; overrides `schema.primaryKey`. postgres-cdc requires a PK."},
    {"name":"schemaName","required":false,"type":"string","doc":"Postgres schema → `'schema-name'`. Defaults to `'public'`."},
    {"name":"slotName","required":false,"type":"string","doc":"Logical replication slot → `'slot.name'`. Defaults to `'flink'`."},
    {"name":"snapshotMode","required":false,"type":"string","doc":"Optional → `'debezium.snapshot.mode'` (e.g. `'never'` to skip the initial snapshot)."},
    {"name":"sslMode","required":false,"type":"string","doc":"Optional → `'debezium.database.sslmode'` (e.g. `'require'` for TLS)."},
    {"name":"tap","required":false,"type":"boolean | TapConfig","doc":"Enable operator tailing for this source."},
  ],
}
