...
const log = createServerLogger().getSubLogger({ name: "api:jars" })

export async function GET() {
  const config = getConfig()

  if (config.mockMode) {
    log.info("MOCK → generateMockJarsApiResponse", {
      screen: "JAR Management",
      file: "mock-api-responses.ts",
      generator: "generateMockJarsApiResponse",
    })
...
export async function POST(request: Request) {
  ...
  if (config.mockMode) {
    log.info("MOCK → generateMockJarUploadApiResponse", {
      screen: "JAR Upload",
      file: "mock-api-responses.ts",
      generator: "generateMockJarUploadApiResponse",
    })
