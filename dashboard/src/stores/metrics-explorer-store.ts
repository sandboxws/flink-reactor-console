...
async function pollMetrics(
  set: (fn: (state: MetricsExplorerState) => Partial<MetricsExplorerState>) => void,
  ...
  const grouped = new Map<string, { source: MetricSource; metricNames: string[] }>();
  ...
  set((state) => {
    const updatedSeries = state.series.map((s) => {
      ...
      const newMin =
        s.minValue === null ? value : Math.min(s.minValue, value);
      const newMax =
        s.maxValue === null ? value : Math.max(s.maxValue, value);
...
export const useMetricsExplorerStore = create<MetricsExplorerState>(
  (set, get) => ({
    ...
    selectSource: (source) => {
      set({ selectedSource: source, availableMetrics: [], metricsLoading: true });
    ...
    addMetric: (source, metricName) => {
      ...
      ringBuffers.set(seriesId, new RingBuffer<MetricDataPoint>(MAX_DATA_POINTS));
