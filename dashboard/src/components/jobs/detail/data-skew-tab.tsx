...
function SkewTooltip({
  ...
  active?: boolean;
  payload?: Array<{ value: number; payload: { subtask: string; records: number; isSkewed: boolean } }>;
}) {
...
export function DataSkewTab({
  ...
  if (vertices.length === 0) {
    return (
      <EmptyState icon={BarChart3} message="No vertex data available" />
    );
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="glass-card p-4">
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
            ...
            <Bar dataKey="records" radius={[3, 3, 0, 0]} isAnimationActive={false}>
              {chartData.map((entry) => (
                <Cell
                  key={entry.subtask}
                  fill={entry.isSkewed ? "var(--color-job-failed)" : "var(--color-fr-purple)"}
      ...
      <div className="grid grid-cols-3 gap-2 lg:grid-cols-6">
        {[
          ...
          <div key={stat.label} className="glass-card flex flex-col items-center gap-0.5 p-2">
