...
function generateTmStdout(tm: TaskManager): string {
  return [
    ...
    `  Max heap size: ${Math.round(tm.metrics.heapMax / (1024 ** 2))} MB`,
    `  JVM args: -Xmx${Math.round(tm.metrics.heapMax / (1024 ** 2))}m -Xms${Math.round(tm.metrics.heapMax / (1024 ** 2))}m -XX:MaxMetaspaceSize=${Math.round(tm.memoryConfiguration.jvmMetaspace / (1024 ** 2))}m -XX:MaxDirectMemorySize=${Math.round(tm.memoryConfiguration.networkMemory / (1024 ** 2))}m`,
    ...
    `JVM heap size: ${Math.round(tm.metrics.heapMax / (1024 ** 2))} MB`,
    `Managed memory: ${Math.round(tm.memoryConfiguration.managedMemory / (1024 ** 2))} MB`,
    `Network memory: ${Math.round(tm.memoryConfiguration.networkMemory / (1024 ** 2))} MB`,
    ``,
