...
function findServerJs(): string {
  ...
  console.error("  Run `pnpm build:package` first to generate the standalone build.");
  process.exit(1);
}
...
program
  ...
  .action(
    (options: {
      ...
      const clusterName =
        process.env.CLUSTER_DISPLAY_NAME || "Default Cluster";
      ...
      console.log();
      console.log(
        "\x1b[36m◆ Flink Reactor Dashboard\x1b[0m",
      );
      console.log();
