...
function loadFromResolvedJson(path: string): Partial<DashboardConfig> {
  ...
  if (json.clusterDisplayName)
    result.clusterDisplayName = json.clusterDisplayName
  if (json.mockMode != null) result.mockMode = json.mockMode
  if (json.rbacEnabled != null) result.rbacEnabled = json.rbacEnabled
  if (json.rbacProvider)
    result.rbacProvider = parseRbacProvider(json.rbacProvider)
  if (json.rbacRoles) result.rbacRoles = json.rbacRoles
  if (json.prometheusUrl) result.prometheusUrl = json.prometheusUrl
  if (json.prometheusEnabled != null)
    result.prometheusEnabled = json.prometheusEnabled
  if (json.alertWebhookUrl) result.alertWebhookUrl = json.alertWebhookUrl
  if (json.alertWebhookEnabled != null)
    result.alertWebhookEnabled = json.alertWebhookEnabled
...
export function getConfig(): DashboardConfig {
  ...
  if (resolvedJsonPath) {
    ...
    config = {
      ...
      flinkRestUrl: process.env.FLINK_REST_URL
        ? envConfig.flinkRestUrl
        : (jsonConfig.flinkRestUrl ?? envConfig.flinkRestUrl),
      dashboardPort: process.env.DASHBOARD_PORT
        ? envConfig.dashboardPort
        : (jsonConfig.dashboardPort ?? envConfig.dashboardPort),

      authType: process.env.FLINK_AUTH_TYPE
        ? envConfig.authType
        : (jsonConfig.authType ?? envConfig.authType),
      authUsername: process.env.FLINK_AUTH_USERNAME
        ? envConfig.authUsername
        : (jsonConfig.authUsername ?? envConfig.authUsername),
      authPassword: process.env.FLINK_AUTH_PASSWORD
        ? envConfig.authPassword
        : (jsonConfig.authPassword ?? envConfig.authPassword),
      authToken: process.env.FLINK_AUTH_TOKEN
        ? envConfig.authToken
        : (jsonConfig.authToken ?? envConfig.authToken),

      sslEnabled: process.env.FLINK_SSL_ENABLED
        ? envConfig.sslEnabled
        : (jsonConfig.sslEnabled ?? envConfig.sslEnabled),
      sslCaPath: process.env.FLINK_SSL_CA_PATH
        ? envConfig.sslCaPath
        : (jsonConfig.sslCaPath ?? envConfig.sslCaPath),

      pollIntervalMs: process.env.DASHBOARD_POLL_INTERVAL
        ? envConfig.pollIntervalMs
        : (jsonConfig.pollIntervalMs ?? envConfig.pollIntervalMs),
      logBufferSize: process.env.DASHBOARD_LOG_BUFFER_SIZE
        ? envConfig.logBufferSize
        : (jsonConfig.logBufferSize ?? envConfig.logBufferSize),

      clusterDisplayName: process.env.CLUSTER_DISPLAY_NAME
        ? envConfig.clusterDisplayName
        : (jsonConfig.clusterDisplayName ?? envConfig.clusterDisplayName),
      clusters: process.env.FLINK_CLUSTERS
        ? envConfig.clusters
        : envConfig.clusters,

      rbacEnabled: process.env.DASHBOARD_RBAC_ENABLED
        ? envConfig.rbacEnabled
        : (jsonConfig.rbacEnabled ?? envConfig.rbacEnabled),
      rbacProvider: process.env.DASHBOARD_RBAC_PROVIDER
        ? envConfig.rbacProvider
        : (jsonConfig.rbacProvider ?? envConfig.rbacProvider),
      rbacRoles: process.env.DASHBOARD_RBAC_ROLES
        ? envConfig.rbacRoles
        : (jsonConfig.rbacRoles ?? envConfig.rbacRoles),

      prometheusUrl: process.env.PROMETHEUS_URL
        ? envConfig.prometheusUrl
        : (jsonConfig.prometheusUrl ?? envConfig.prometheusUrl),
      prometheusEnabled: process.env.PROMETHEUS_ENABLED
        ? envConfig.prometheusEnabled
        : (jsonConfig.prometheusEnabled ?? envConfig.prometheusEnabled),

      alertWebhookUrl: process.env.ALERT_WEBHOOK_URL
        ? envConfig.alertWebhookUrl
        : (jsonConfig.alertWebhookUrl ?? envConfig.alertWebhookUrl),
      alertWebhookEnabled: process.env.ALERT_WEBHOOK_ENABLED
        ? envConfig.alertWebhookEnabled
        : (jsonConfig.alertWebhookEnabled ?? envConfig.alertWebhookEnabled),

      mockMode: process.env.DASHBOARD_MOCK_MODE
        ? envConfig.mockMode
        : (jsonConfig.mockMode ?? envConfig.mockMode),
    }
  }
