...
import { generateEmbeddings, generateEmbedding, checkModel } from "./embed.js";
...
const TEST_QUERIES: TestQuery[] = [
  ...
  { query: "Button component with different variants", expectedFiles: ["button.tsx"], category: "direct" },
  { query: "Dialog modal popup overlay", expectedFiles: ["dialog.tsx"], category: "direct" },
  { query: "Tabs component tabbed interface", expectedFiles: ["tabs.tsx"], category: "direct" },
  { query: "Progress bar indicator", expectedFiles: ["progress.tsx"], category: "direct" },

  // Feature-based search
  { query: "collapsible sidebar with navigation groups", expectedFiles: ["sidebar.tsx"], category: "feature" },
  { query: "command palette with keyboard shortcut search", expectedFiles: ["command-palette.tsx"], category: "feature" },
  { query: "text viewer with line numbers and copy", expectedFiles: ["text-viewer.tsx"], category: "feature" },
  { query: "resizable split panels layout", expectedFiles: ["resizable.tsx"], category: "feature" },

  // Props/API search
  { query: "MetricCard props for displaying statistics with icon", expectedFiles: ["metric-card.tsx"], category: "props" },
  { query: "SearchInput with regex mode toggle", expectedFiles: ["search-input.tsx"], category: "props" },
  { query: "TimeRange presets for 5 minutes 15 minutes 1 hour", expectedFiles: ["time-range.tsx"], category: "props" },
  { query: "Sidebar LinkComponent prop for custom router", expectedFiles: ["sidebar.tsx"], category: "props" },

  // Style/theming search
  { query: "Tokyo Night theme color palette brand colors", expectedFiles: ["tokens.css"], category: "style" },
  { query: "job status colors running finished cancelled failed", expectedFiles: ["tokens.css"], category: "style" },
  { query: "glass card glassmorphism backdrop blur effect", expectedFiles: ["components.css"], category: "style" },
  { query: "log severity badge styling trace debug info warn error", expectedFiles: ["components.css", "severity-badge.tsx"], category: "style" },

  // Conceptual/use-case search
  { query: "display empty placeholder when no data available", expectedFiles: ["empty-state.tsx"], category: "conceptual" },
  { query: "breadcrumb navigation from URL path segments", expectedFiles: ["header.tsx"], category: "conceptual" },
  { query: "hoverable card with preview content", expectedFiles: ["hover-card.tsx"], category: "conceptual" },
  { query: "application shell with sidebar and header", expectedFiles: ["shell.tsx"], category: "conceptual" },
];
...
async function main(): Promise<void> {
  ...
  for (const model of MODELS) {
    ...
    if (!ready) {
      console.error(`  ✗ Model "${model.ollamaModel}" not found. Run: ollama pull ${model.ollamaModel}`);
      process.exit(1);
    }
  ...
  for (const model of MODELS) {
    ...
    console.log(`    Avg embed time: ${(totalEmbedTime / records.length).toFixed(0)}ms/chunk`);
    ...
    try {
      ...
      if (msg.includes("Failed to connect") || msg.includes("ECONNREFUSED")) {
        console.log(`\n  ── chromadb ──`);
        console.log(`    ⚠ ChromaDB server not running (requires: chroma run --path <dir>)`);
        console.log(`    ⚠ Skipping — ChromaDB JS client cannot run in embedded mode.`);
        console.log(`    ⚠ This is a key trade-off: LanceDB is fully embedded, ChromaDB is not.\n`);
      } else {
        throw err;
      }
    }
...
async function benchmarkDb(
  ...
  console.log(`    P@1: ${(benchResult.metrics.precision_at_1 * 100).toFixed(1)}%`);
  console.log(`    P@3: ${(benchResult.metrics.precision_at_3 * 100).toFixed(1)}%`);
