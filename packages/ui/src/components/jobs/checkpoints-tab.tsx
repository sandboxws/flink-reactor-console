...
function ExpandableOperatorRow({
  ...
  const handleToggle = () => {
    if (!expanded && !subtasks && !loading) {
      setLoading(true)
      setError(null)
      fetchSubtaskDetail(jobId, checkpointId, vid)
        .then((data) => {
          setSubtasks(data)
          setExpanded(true)
        })
...
function CheckpointDetailView({
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    fetchDetail(jobId, checkpointId)
      .then((d) => {
  ...
  }, [jobId, checkpointId, fetchDetail])
            ...
            <tbody>
              {tasks.map(([vid, t]) => (
                <ExpandableOperatorRow
                  ...
                  fetchSubtaskDetail={fetchSubtaskDetail}
                />
              ))}
            </tbody>
          </table>
  ...
  if (selectedCheckpoint !== null) {
    return (
      <CheckpointDetailView
        ...
        fetchDetail={fetchCheckpointDetail}
        fetchSubtaskDetail={fetchCheckpointSubtaskDetail}
      />
    )
  }
